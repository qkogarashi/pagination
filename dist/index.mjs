...
function GeneratePage(item, page, maxPage, config) {
  ...
  if (config.type === 0 /* Button */) {
    const startBtn = new ButtonBuilder().setCustomId(config.start?.id ?? defaultIds.buttons.start).setStyle(config.start?.style ?? ButtonStyle.Primary).setDisabled(beginning);
    const endBtn = new ButtonBuilder().setCustomId(config.end?.id ?? defaultIds.buttons.end).setStyle(config.end?.style ?? ButtonStyle.Primary).setDisabled(end);
    const nextBtn = new ButtonBuilder().setCustomId(config.next?.id ?? defaultIds.buttons.next).setStyle(config.next?.style ?? ButtonStyle.Primary).setDisabled(end);
    const prevBtn = new ButtonBuilder().setCustomId(config.previous?.id ?? defaultIds.buttons.previous).setStyle(config.previous?.style ?? ButtonStyle.Primary).setDisabled(beginning);
    const exitBtn = new ButtonBuilder().setCustomId(config.exit?.id ?? defaultIds.buttons.exit).setStyle(config.exit?.style ?? ButtonStyle.Danger);
  ...
  const menu = new StringSelectMenuBuilder().setCustomId(config.menuId ?? defaultIds.menu).setPlaceholder(config.placeholder ?? "Select page").setOptions(paginator);
  ...
  */
  async start() {
    ...
    if (page.newMessage.components) {
      ...
      page.newMessage.components = this.option.isV2Components ? [new ContainerBuilder().addActionRowComponents(page.paginationRow)] : [page.paginationRow];
    }
    let message;
  ...
  async createCollector(message) {
    const collector = message.createMessageComponentCollector({
      ...this.option,
      componentType: this.option.type === 0 /* Button */ ? ComponentType.Button : ComponentType.StringSelect,
    ...
    collector.on("collect", async (collectInteraction) => {
      if (collectInteraction.isButton() && this.option.type === 0 /* Button */) {
        ...
        if (pageEx.newMessage.components) {
          ...
          pageEx.newMessage.components = this.option.isV2Components ? [new ContainerBuilder().addActionRowComponents(pageEx.paginationRow)] : [pageEx.paginationRow];
        }
